...
public class FlowLayout extends LinearLayout {
	...
	public FlowLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				com.wefika.flowlayout.R.styleable.FlowLayout, defStyle, 0);

        try {
            int index = a.getInt(com.wefika.flowlayout.R.styleable.FlowLayout_android_gravity, -1);
            if(index > 0) {
                setGravity(index);
            }
        } finally {
...
/*
	@Override
	protected RadioGroup.LayoutParams generateLayoutParams(RadioGroup.LayoutParams p) {
